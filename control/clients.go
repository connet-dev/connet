...
func (c *clientConn) run(ctx context.Context) {
	...
	defer c.conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_Unknown), "connection closed")
...
func (c *clientConn) runErr(ctx context.Context) error {
	if auth, id, err := c.authenticate(ctx); err != nil {
		if perr := pberror.GetError(err); perr != nil {
			...
			// TODO handle err
			c.conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_AuthenticationFailed), "Error while authenticating")
		}
		return err
	} else {
		c.auth = auth
...
func (c *clientConn) authenticate(ctx context.Context) (ClientAuthentication, ksuid.KSUID, error) {
	...
	if err != nil {
		...
		if perr == nil {
			perr = pberror.NewError(pberror.Error_AuthenticationFailed, "authentication failed: %v", err)
		}
	...
	if err != nil {
		err := pberror.NewError(pberror.Error_AuthenticationFailed, "cannot resolve origin: %v", err)
...
func validatePeerCert(fwd model.Forward, peer *pbclient.Peer) *pberror.Error {
	if _, err := x509.ParseCertificate(peer.ClientCertificate); err != nil {
		return pberror.NewError(pberror.Error_AnnounceInvalidClientCertificate, "'%s' client cert is invalid", fwd)
	}
	if _, err := x509.ParseCertificate(peer.ServerCertificate); err != nil {
		return pberror.NewError(pberror.Error_AnnounceInvalidServerCertificate, "'%s' server cert is invalid", fwd)
	}
	return nil
}
...
func (s *clientStream) announce(ctx context.Context, req *pbclient.Request_Announce) error {
	...
	if newFwd, err := s.conn.server.auth.Validate(s.conn.auth, fwd, role); err != nil {
		perr := pberror.GetError(err)
		if perr == nil {
			perr = pberror.NewError(pberror.Error_AnnounceValidationFailed, "failed to validate forward '%s': %v", fwd, err)
		}
	...
	g.Go(func() error {
		for {
			...
			if req.Announce == nil {
				respErr := pberror.NewError(pberror.Error_RequestUnknown, "unexpected request")
...
func (s *clientStream) relay(ctx context.Context, req *pbclient.Request_Relay) error {
	...
	if newFwd, err := s.conn.server.auth.Validate(s.conn.auth, fwd, role); err != nil {
		perr := pberror.GetError(err)
		if perr == nil {
			perr = pberror.NewError(pberror.Error_RelayValidationFailed, "failed to validate desination '%s': %v", fwd, err)
		}
		if err := proto.Write(s.stream, &pbclient.Response{Error: perr}); err != nil {
	...
	if err != nil {
		err := pberror.NewError(pberror.Error_RelayInvalidCertificate, "invalid certificate: %v", err)
		if err := proto.Write(s.stream, &pbclient.Response{Error: err}); err != nil {
...
func (s *clientStream) unknown(_ context.Context, req *pbclient.Request) error {
	...
	err := pberror.NewError(pberror.Error_RequestUnknown, "unknown request: %v", req)
