...
func (c *clientConn) runErr(ctx context.Context) error {
	...
	if auth := c.server.auth(serverName, certs); auth == nil {
		return c.conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_AuthenticationFailed), "authentication missing")
	} else {
		c.auth = auth
...
func (c *clientConn) connect(ctx context.Context, stream quic.Stream, fcs *forwardClients, req *pbconnect.Request) error {
	...
	if len(dests) == 0 {
		err := pberror.NewError(pberror.Error_DestinationNotFound, "could not find destination")
	...
	err := pberror.NewError(pberror.Error_DestinationDialFailed, "could not dial destinations: %v", pberrs)
	return proto.Write(stream, &pbconnect.Response{Error: err})
}
