...
func (p *directPeer) run(ctx context.Context) {
	defer func() {
		...
		for _, conn := range active {
			defer conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_DirectConnectionClosed), "connection closed")
		}
	}()
...
func (p *directPeer) runRemote(ctx context.Context) error {
	return p.remote.Listen(ctx, func(remote *pbclient.RemotePeer) error {
		...
		var remotes = map[relayID]struct{}{}
...
func (p *directPeerIncoming) keepalive(ctx context.Context, conn quic.Connection) error {
	defer conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_DirectKeepaliveClosed), "keepalive closed")
...
func (p *directPeerOutgoing) connect(ctx context.Context) (quic.Connection, error) {
	...
	for paddr := range p.addrs {
		...
		case errors.Is(err, context.Canceled):
			return nil, err
		case err != nil:
			conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_ConnectionCheckFailed), "connection check failed")
			errs = append(errs, err)
			continue
		}
