...
func (s *DirectServer) runConn(conn quic.Connection) {
	...
	if srv == nil {
		conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_AuthenticationFailed), "unknown server")
		return
	}
	...
	if exp == nil {
		conn.CloseWithError(quic.ApplicationErrorCode(pberror.Error_AuthenticationFailed), "unknown client")
		return
	}
