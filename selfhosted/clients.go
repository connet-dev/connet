...
func (s *clientsAuthenticator) Authenticate(req control.ClientAuthenticateRequest) (control.ClientAuthentication, error) {
	...
	if !ok {
		return nil, pberror.NewError(pberror.Error_AuthenticationFailed, "token not found")
	}
	if !r.IPs.IsAllowedAddr(req.Addr) {
		return nil, pberror.NewError(pberror.Error_AuthenticationFailed, "address not allowed: %s", req.Addr)
	}
...
func (s *clientsAuthenticator) Validate(auth control.ClientAuthentication, fwd model.Forward, role model.Role) (model.Forward, error) {
	...
	if !ok {
		return model.Forward{}, pberror.NewError(pberror.Error_AuthenticationFailed, "token not found")
	}
	if !r.Names.IsAllowed(fwd.String()) {
		return model.Forward{}, pberror.NewError(pberror.Error_ForwardNotAllowed, "forward not allowed: %s", fwd)
	}
	if r.Role != model.UnknownRole && r.Role != role {
		return model.Forward{}, pberror.NewError(pberror.Error_RoleNotAllowed, "role not allowed: %s", role)
	}
	return fwd, nil
}
